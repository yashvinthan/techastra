...
export default class MonitorScreen extends EventEmitter {
    ...
    createIframe() {
        ...
        // Handle cross-frame proctoring and seamless fullscreen OS transitions
        window.addEventListener('message', (event) => {
            if (event.data?.type === 'CODE_RESCUE_ENTER_FULLSCREEN') {
                document.body.classList.add('fullscreen-os-active');
                if (!document.fullscreenElement) {
                    document.documentElement.requestFullscreen().catch(() => {});
                }
                return;
            }
            if (event.data?.type === 'CODE_RESCUE_EXIT_FULLSCREEN') {
                document.body.classList.remove('fullscreen-os-active');
                if (document.fullscreenElement) {
                    document.exitFullscreen().catch(() => {});
                }
                // Transition camera back to monitor / desk view
                this.camera.trigger('enterMonitor');
                return;
            }
        });
        ...
        if (urlParams.has('src')) {
            iframe.src = urlParams.get('src')!;
        } else if (urlParams.has('dev') || (isLocal && !urlParams.has('static'))) {
            // Local dev mode: load live React dev server on port 3000
            iframe.src = 'http://localhost:3000/';
        } else {
            // Production deployment (GitHub Pages) or ?static=true:
            // Loads self-contained Windows 95 OS bundled at ./os/
            iframe.src = './os/';
        }
